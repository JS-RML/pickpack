...
ros::Publisher pressure_publish("pressure", &str_msg);
...
void setup () {
  ...
  pres.initNode();
  pres.advertise(pressure_publish);
...
void loop() {
  ...
  for(int i=0;i<addressLength;i++)
  {
    ...
    pressure_string.concat(int(oPressure));
    }
  ...
  char pressure_string0[50];
  pressure_string.toCharArray(pressure_string0, 50);
  str_msg.data = pressure_string0;
  pressure_publish.publish( &str_msg );
  pres.spinOnce();
  //delay(1000);
